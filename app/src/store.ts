...
import {
  ...
  arestaDoQuadranteMorto, tokenQuadranteVivo,
} from "./lib/galho";
import {
...
export const useStore = create<ComposerState>()(
  ...
  criarGalho: (p) => {
    ...
    if (ehAlca) {
      const tokenVivo = tokenQuadranteVivo(p.sentido);
  ...
  rebuildIntersectionCorridors: (intId: string) => { console.time('rebuildIntersectionCorridors');
    ...
    const ladoAccel = (side: 1 | -1) => {
      ...
      /* ONDE A FAIXA ADICIONAL ENCOSTA NA CONCORDÂNCIA — CORRIGIDO PELO VIÉS.
       *
       * A soma antiga (`branchLaneW + R`) só vale a 90°. Enviesado, a meia-largura
       * do ramo deixa de medir ao longo do eixo da principal: projetada, vira
       * `branchLaneW / |sen α|`. E a perna da tangente do fillete não é R, é
       * `R / tg(α/2)` — a fórmula da tangente do arco entre dois alinhamentos que
       * se cruzam em α. A 90° as duas se reduzem ao caso antigo
       * (sen 90° = 1, tg 45° = 1), então só muda o que estava errado.
       *
       * Ângulo preso longe de 0 e de π: no limite as duas parcelas divergem, e
       * uma faixa adicional de estaca infinita destrói o corredor. */
      const R = (side === 1 ? int.rightRadius : int.leftRadius) || 15;
      const mDirLado =
        side === 1 ? mainUnitDir : { x: -mainUnitDir.x, y: -mainUnitDir.y };
      const dot = mDirLado.x * branchUnitDir.x + mDirLado.y * branchUnitDir.y;
      const cross = mDirLado.x * branchUnitDir.y - mDirLado.y * branchUnitDir.x;
    ...
    try {
      ...
      /* ALÇA: O QUADRANTE DESCARTADO NÃO EXISTE.
       *
       * `buildIntersectionPolygon` recebe três braços e devolve os três cantos —
       * é o entroncamento completo, sempre. Na alça um dos quadrantes do ramo foi
       * descartado, e a aresta dele continuava a ser emitida: é a linha preta que
       * corta o gore de um nariz ao outro.
       *
       * Filtrar aqui resolve os três sintomas de uma vez, porque tudo a jusante
       * lê desta lista: a linha desaparece do desenho, os dois narizes daquele
       * lado deixam de ser detectados (eram cruzamentos COM ela), e a tangente
       * fantasma sai de `mainTangents` — que é quem alimenta a garganta e a faixa
       * adicional. Apagar nariz por nariz trataria sintoma. */
